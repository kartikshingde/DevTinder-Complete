...
const Chat = () => {
  ...
  const fetchChatMessages = async () => {
    ...
    console.log("All participants:", chat.data.participants); // Debug line
  console.log("Current userId:", userId); // Debug line

    // Extract target user from participants (the one who is NOT the current user)
    const target = chat.data.participants?.find(
      participant => participant._id !== userId
    );

     console.log("Target user found:", target); // Debug line
    
    setTargetUser(target); // Store target user info
  ...
  return (
    <div className="w-full max-w-5xl mx-auto h-[100dvh] md:h-[75vh] flex flex-col bg-base-200 md:rounded-2xl md:shadow-2xl overflow-hidden md:my-6 md:border md:border-base-300">
      
      {/* Header - Modern Gradient Design */}
      <div className="px-4 sm:px-6 py-4 bg-gradient-to-r from-primary via-secondary to-accent border-b border-base-300/50 shadow-lg">
          ...
          <div className="avatar online placeholder">
            <div className="w-11 sm:w-12 rounded-full bg-base-100 ring ring-primary ring-offset-2 ring-offset-base-200">
              <span className="text-xl sm:text-2xl font-bold text-primary">
                {targetUser?.firstName?.charAt(0).toUpperCase() || 'C'}
              </span>
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-base-100" style={{ scrollBehavior: 'smooth' }}>
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-base-content/50">
            ...
            <p className="text-base sm:text-lg font-semibold mb-1">No messages yet</p>
            <p className="text-xs sm:text-sm text-center px-4">Start the conversation by sending a message</p>
          </div>
        ) : (
          <>
              ...
              return (
                ...
                >
                  ...
                  <div className="chat-header text-sm sm:text-base font-semibold mb-1">
                    ...
                    <time className="text-xs opacity-60 ml-2">12:45</time>
                  </div>
                  <div 
                    className={`chat-bubble text-sm sm:text-base shadow-lg max-w-[85%] sm:max-w-xs md:max-w-md break-words whitespace-pre-wrap ${isMe ? "chat-bubble-primary" : "chat-bubble-secondary"}`}
                  >
                    {msg.text}
                  </div>
      ...
      <div className="px-3 sm:px-6 py-3 sm:py-4 bg-base-200 border-t border-base-300 shadow-lg shrink-0">
        <div className="flex items-end gap-2 sm:gap-3">
          ...
          <button 
            onClick={sendMessage} 
            className={`btn btn-primary btn-circle shadow-lg transition-transform hover:scale-110 ${!newMessage.trim() ? 'btn-disabled opacity-50' : ''}`}
